        ...
        <div class="info">
          ...
          <div class="shopCart">
            <transition name="fade">
              <div class="text" @click="addCart($event)" v-show="!food.count||food.count===0">加入购物车</div>
            </transition>
          </div>
          <div class="warpc"  v-show="food.count||food.count!=0">
...
<!--          </div>-->
<!--        </div>-->
        <Reating :selectType="selectType" :onlyContent="onlyContent" :desc="classifyArr" :food="food" ></Reating>
      </div>
    </div>
  </transition>
</template>
...
import Reating from '../reatingselete/reatingselete'
const ALL = 2
export default {
  components: {
    Split,
  ...
  components: {Split, Reating,cartcontrol},
  methods: {
    showToggle () {
